...
%% OutputF5 is the optimal solution we need
% clear all
% close all
% clc
...
InpFile = '180219pH_Nesb10_NET40_16.mat';
...
%  read data
 prodata = xlsread('F:\paper result\pHT\paper supplement\original data.xlsx','Raw data');
...
prodata_N= [sin(Lon_n) cos(Lon_n) sin(month_n) cos(month_n) Lat_n T S DO NO3 PO4 SI];
...
inputsF = prodata_N(:,1:11)'; %% input
% targetsF = prodata_N(:,9)';%% output pH

%% Simulate
outputsF = sim (eval(strcat('net',num2str(i))), inputsF ); 
outputsF=1.5*outputsF*sd(12)+av(12); % 
